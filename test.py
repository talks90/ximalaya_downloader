...
class XimaScraper:
    def __init__(self, album_no, page_num):
        ...
        url = base_url + 'album?albumId={}&pageNum={}&sort={}&pageSize={}'.format(album_no, page_num, sort_order,
                                                                                  album_size)
        print(url)
        req = requests.get(url, headers=headers)
        self.full_tracks_info = req.json()['data']['tracksAudioPlay']
        self.album_name = self.full_tracks_info[0]['albumName']
        self.has_more = req.json()['data']['hasMore']
...
if __name__ == '__main__':
    ...
    page_num = 1
    xima = XimaScraper(album_no, page_num)
    filedir = xima.album_name
    has_more = xima.has_more
    while bool(has_more) is True:
        xima = XimaScraper(album_no, page_num)
        filedir = xima.album_name
        has_more = xima.has_more
