...
class Net:
    ...
    def Get(self, url, header_string="", cookie="", SSL_verify=0, stream=0):

        header_dict = self.get_header_dict(header_string)

        rs = requests.get(url, headers=header_dict, verify=SSL_verify, cookies=cookie, stream=stream)
    ...
    def Download(self, url, local_filename=None):
        ...
        with open(local_filename, 'wb') as f:
            for chunk in r.iter_content(chunk_size=1024):       #8192
